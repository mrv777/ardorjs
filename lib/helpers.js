...
var EPOCH_BEGINNING_TESTNET = 1514296800000;
...
function byteArrayToBigInteger(bytes, opt_startIndex) {
  ...
  for (var i = 7; i >= 0; i--) {
    temp1 = value.multiply(new BigInteger("256", 10));
    temp2 = temp1.add(
      new BigInteger(bytes[opt_startIndex + i].toString(10), 10)
    );
    value = temp2;
  }
...
function wordArrayToByteArray(wordArray, isFirstByteHasSign) {
...
function toEpochTime(currentTime, isTestnet) {
  if (currentTime == undefined) {
    currentTime = new Date();
  }

  return isTestnet
    ? Math.floor((currentTime - EPOCH_BEGINNING_TESTNET) / 1000)
    : Math.floor((currentTime - EPOCH_BEGINNING) / 1000);
}
