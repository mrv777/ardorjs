...
function verifyTransactionBytes(byteArray, requestType, data, publicKey) {
  ...
  pos += 2;
  transaction.publicKey = helpers.byteArrayToHexString(
    byteArray.slice(pos, pos + 32)
  );
  pos += 32;
  ...
  if (data.deadline) {
    // Only check deadline if it was provided
  ...
  if (
    ...
    if (
      ...
      data.recipient.substring(data.recipient.indexOf("-"))
    ) {
      return false;
    }
  }
  ...
  if (
    Number(transaction.amountNQT) !== Number(data.amountNQT) &&
    !(requestType === "exchangeCoins" && transaction.amountNQT === "0")
  ) {
    return false;
  }
